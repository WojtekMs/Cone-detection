...
def postprocess(output: np.ndarray) -> "tuple[np.ndarray, np.ndarray, np.ndarray]":
    ...
    number_of_detections = tensor_output[0]
    detection_per_row = tensor_output[1:].reshape(-1, 6)[:int(number_of_detections)]
